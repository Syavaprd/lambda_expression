...
parse :: [Token] -> Expr
parse [] = Empty
parse (LOpen : xs) = App (parse (fst (appSplit (split 'L' xs 0)))) (parse (snd (appSplit (split 'L' xs 0))))
...
lazy :: Expr -> String
lazy x = if exprStr x /= exprStr (exprLazy x) then lazy (exprLazy x) else exprStr x
...
reduce str = lazy (fst (alphaConversion (parse (lStep str)) 0))